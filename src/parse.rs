...
pub trait Parsable : Sized {
    fn parser () -> impl Parser<Dest=Self>;
}
...
mod stdp { // parsers for std types
    ...
    impl Parser for I32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
            ...
            Ok((
                &input[end_idx..],
                NonZeroI32::new(value).ok_or(())?.get()
            ))
        }
    }
...
impl Parsable for AuthData {
    fn parser () -> impl Parser<Dest = Self> {
        map(
            take(AUTHDATA_SIZE, stdp::Byte),
            |authdata| AuthData(
                authdata.into_boxed_slice().try_into().unwrap_or_else(|_|
                    vec![0;AUTHDATA_SIZE].into_boxed_slice().try_into().unwrap()
                )
            )
        )
    }
...
pub fn parse<T: Parsable>(input: &str) -> Result<(&str, T), ()> {
