...
impl<T: std::io::Read> Iterator for LogIterator<T> {
    type Item = parse::LogLine;
    fn next(&mut self) -> Option<Self::Item> {
        let line = self.lines.next()?.ok()?;
        let (remaining, result) = parse(line.trim()).ok()?;
        remaining.trim().is_empty().then_some(result)
    }
}

/// Принимает поток байт, отдаёт отфильтрованные и распарсенные логи
pub fn read_log<T: std::io::Read>(input: T, mode: ReadMode, request_ids: Vec<u32>) -> Vec<LogLine> {
